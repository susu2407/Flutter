...
class _CategoryTabState extends State<CategoryTab> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Category>>( // _categories(Future 객체)가 완료될 때까지 기다렸다가,그 결과(snapshot)를 기반으로 UI를 그려라
        future: _categories,            // 감시 대상 (비동기 작업)
        builder: (context, snapshot) {  // UI를 그리는 함수(Future의 상태 변화에 따라 Flutter가 자동으로 호출하는 콜백 함수)
                            // snapshot은 future의 현재 상태
