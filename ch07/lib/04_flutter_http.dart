...
// 직접 입력
import 'package:http/http.dart' as http; // as 별칭
...
class _HttpScreenState extends State<HttpScreen> {
  ...
  Future<void> fetchGet(){
    ...
    // http 의존성 추가 : pub.dev 검색 > http: ^1.5.0
    return http.get(url, headers: {'Content-Type': 'application/json'})
      .then((response){
         if(response.statusCode == 200) {
           var jsonResult = jsonDecode(response.body); // 서버의 바디로 들어온다?
           setState(() {
  ...
  Future<void> fetchPost() async {
    ...
    try {
      final response = await http.post(url,
        ...
        body: jsonEncode(
          { // (지금은 여기에다 json을 바로 작성했지만, 나중에는 from을 이용해서 json을 걸어두면 됨)
            "userId": 1001,
